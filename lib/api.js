...
module.exports = class API {
    ...
    async listAvailable() {
        ...
        try {
            ...
            let rows = [ ["Number" , "Available Data"] ];
            let i = 0;
    ...
    async currentElectricty() {
        ...
        try {
            ...
            //Traitement
            const electricity = data.payload.electricity
            const current = electricity.current
            const voltage = electricity.voltage
            const power = electricity.power

            let rows = [ 
                ["Courant en mA" , current],
                ["Voltage en dV" , voltage],
                ["Puissance en mW" , power]
            ];
    ...
    async consumption() {
        ...
        try {
            ...
            //Traitement
            const consumptionx = data.payload.consumptionx

            let rows = [];
            for (let key in consumptionx) {
                rows.push([ key.date , key.value])
                //rows.push("{"+ i + " : " + key + "}");
              }
